...
check_hyper_before_install() {
  if (command_exist hyperctl hyperd);then
    echo "${WHITE}"
    cat <<COMMENT
Prompt: "hyper-container" appears to have already installed, hyperd service will be restarted during installation.
...
install_from_rpm(){
  ...
  case "$1" in
    ...
    fedora23)
      ...
      ;;
    *) show_message error "rpm install supports centos7 & fedora23 only"; exit 1;;
  esac
  set -e
}
display_support() {
